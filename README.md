...
```
`[GenerateReadOnlyStruct]` takes two positional boolean parameters:

* `GenerateConstructor` (defaults to false)
* `GenerateCopyConstructor` (defaults to false)

Due to syntax tree evaluation logic, constructors are **not** generated by default. To generate constructors, you must explicitly pass `true` as positional arguments.
...
If you prefer to implement constructors manually in the original struct, they will be automatically copied and renamed in the generated `ReadOnly` struct.
